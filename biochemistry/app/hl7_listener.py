                        ...
                        if s_type == "obx":
                            ...
                            if result[1] == "NM":
                                ...
                                try:
                                    ...
                                    value = round(float(raw_value), 2)
                                    results.append({"par": result[3], "value": value})
                                except Exception as e:
...
# async def main():
#     try:
#         print(f"Starting HL7 Biochemistry server on {HL7_HOST}:{HL7_PORT}...")
...
async def main():
    ...
    while True:  # <--- Retry loop
        try:
            ...
            print("Will bind to HL7_HOST =", HL7_HOST)
            ...
            print(f"✅ Server started successfully on {HL7_HOST}:{HL7_PORT}")
            async with server:
            ...
            backoff = min(max_backoff, backoff * 2 + random.uniform(0, 1))  # Exponential backoff
...
#     port = 5680
#     print(f"Starting HL7 Biochemistry server on {host}:{port}...")
